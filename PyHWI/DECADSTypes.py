...
class dvmValue():
    def __init__( self, value=None, error=None, unit=None ):
        ...
        if type( value ) is str:
            ...
            # Parse it            
            errorSplit = value.split('g',1)

            if len(errorSplit) == 2:
                # if there is a 'g' in the text, then the first part is the value
                valueStr = errorSplit[0]
                unitSplit = errorSplit[1].split('[',1)
                errorStr = unitSplit[0]
                if len(unitSplit) == 2:
                    unitStr = unitSplit[1].split(']')[0]
                else:
                    unitStr = ''
            else:
                # if there is a 'g' in the text, then the first part is the value
                unitSplit = errorSplit[0].split('[',1)
                valueStr = unitSplit[0]
                errorStr = ''
                if len(unitSplit) == 2:
                    unitStr = unitSplit[1].split(']')[0]
                else:
            ...
            try:
                self.value = float( valueStr )
            except ValueError:
                print "dvmValue initialised with non-floatable value string: %s" % (valueStr)
                raise
            ...
            try:
                if errorStr is '':
                    ...
                    self.error = float( errorStr )
            except ValueError:
                print "dvmValue initialised with non-floatable error string: %s" % (errorStr)
                raise

            self.unit = physUnit( unitStr )
...
class dvmList():
    def __init__( self, n=None, unit=None, values=None, errors=None ):
        ...
        if( n is None or n == 0 ): # initialise empty dvmList
            ...
            if len(listSplit) == 1:
                # If there are no '}' in the string, then there is only one element
                # Parse it as a dvmValue, then copy the entries
                newValue = dvmValue( listSplit[0] )
                self.n = 1
                self.values = [newValue.value]
                self.errors = [newValue.error]
                self.unit = newValue.unit
                return

            valuesSplit = listSplit[0].split('{')[1].split(',')
            self.n = len(valuesSplit)
            self.values = [None]*self.n
            self.errors = [None]*self.n
            # Parse all the values
            for i in range(0, self.n):
                errorSplit = valuesSplit[i].split('g',1)
                if len(errorSplit) == 2:
                    try:
                        self.errors[i] = float( errorSplit[1] )
                    except ValueError:
                        print "dvmList initialised with non-floatable error string in element %i: %s" % (i,errorSplit[1])
                        raise
                try:
                    self.values[i] = float( errorSplit[0] )
                except ValueError:
                    print "dvmList initialised with non-floatable value string in element %i: %s" % (i,errorSplit[0])
                    raise

            unitSplit = listSplit[1].split('[',1)
            if len(unitSplit) == 2:
                self.unit = physUnit( unitSplit[1].split(']',1)[0] )
            else:
                self.unit = None
    ...
    def encode( self, stream ):
        ...
        # If there are no errors, don't send any error bytes
        if self.errors is None:
            return
        # Otherwise send a byte for each error
    ...
    def __str__( self ):
        outputString = "dvmList: " + repr( self.values )
        return outputString    
    def __repr__( self ):
        outputString = "dvmList: " + repr( self.values )
        return outputString   
...
class hwiFunctionList():
    ...
    def __str__( self ):
        output = "There are a total of %i functions:" % (self.n)
        for i in range( 0, self.n ):
