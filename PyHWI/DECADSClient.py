...
# An exception which is raised when something bad happens in the client
class clientException( Exception ):
    pass

# An exception which is raised when the client cannot connect to the given ip/port
class clientConnectionException( clientException ):
    pass

# An exception which is raised when the client cannot find the given alias
class clientBadAliasException( clientException ):
    pass

# An exception which is raised when the client finds the server but is rejected at
# the handshake
class clientRejectedException( clientException ):
    pass

# Raised when there is an error in receiving variable data from
# a server
class clientReceivedBadDECADSDataException( clientException ):
    pass
...
aliasDirectories = [ os.path.expanduser('~/Documents/'), os.path.expanduser('~'), os.path.expanduser('~/My Documents/'), '/' ]
aliasFileName = 'PyHWIAliases.txt'

# Looks up a HWDM alias
# Searches in $HOME/Documents/PyHWIAliases.txt, $HOME/PyHWIAliases.txt, C:/PyHWIAliases.txt and /PyHWIAliases.txt 
# Each line in the alias file should be:
#  AliasName, HWDMName, ip, port
def aliasLookup( alias ):

    for directory in aliasDirectories:
        try:
            directoryListing = os.listdir( directory )
        except WindowsError:
            # If the directory does not exist, windows throws an error
            continue

        if aliasFileName not in directoryListing:
            continue

        aliasFile = open( os.path.join( directory, aliasFileName ), 'r' )
        for line in aliasFile.readlines():
            ...
            if alias != lineSplit[0]:
                continue
            if len( lineSplit) != 4:
                print 'Line has too many terms in alias file: %s' % (line)
                continue

            try:
                retVal = [ lineSplit[1], lineSplit[2], int(lineSplit[3]) ]
            except ValueError:
                print 'Badly formatted line in alias file: ' + line
                continue
...
class DECADSClientConnection():
    def __init__( self, name, address=None, port=None, blocking=True, postInitExecute=None ):
        ...
        # Check for alias
        if address is None:
            aliasEntry = aliasLookup( name )
            if aliasEntry is None:
                raise clientBadAliasException( 'Could not find alias \'%s\'' % (name) )
            name = aliasEntry[0]
            address = aliasEntry[1]
            port = aliasEntry[2]
    ...
    def connect( self, blocking=True, postInitExecute=None ):
        ...
        try:
            ...
            raise clientConnectionException( "Socket error whilst connecting to %s on %s:%i" % \
                                             (self.serverName, self.serverAddress, self.serverPort) )
        except socket.timeout:
            raise clientConnectionException( "Socket timeout whilst connecting to %s on %s:%i" % \
                                 (self.serverName, self.serverAddress, self.serverPort) )
        ...
        try:
            ...
            raise clientRejectedException( "Socket error whilst receiving handshake from %s on %s:%i" % \
                                             (self.serverName, self.serverAddress, self.serverPort)  )
    
        if( acceptedResponse[0:26] != "HIMOL Connection Accepted\0" ):
            self.serverSocket.shutdown(1)
            self.serverSocket.close()
            raise clientRejectedException( "Bad handshake from server %s on %s:%i.\nMessage follows:%s" % \
                                             (self.serverName, self.serverAddress, self.serverPort, acceptedResponse)  )
    ...
    def executeFunction( self, function, inputs, blocking=True ):
        ...
        try:
            ...
            raise clientException( "Socket error in executeFunction %s" % (function.name) )
        ...
        while(1):
            ...
            if( packetHeader == packetHeader_functionListAnswer ): # Server replying with its list of functions
                ...
                try:
                    ...
                    raise clientReceivedBadDECADSDataException( \
                        'Received bad from server in function %s. Error: %s' %\
                        (self.functionList.functions[ functionIndex ].name, str(e) ) )
