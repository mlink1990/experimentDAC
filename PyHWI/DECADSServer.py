...
# An exception which is raised when something bad happens in the server
# I.E. the listening socket cannot be created
...
def simpleServer( hwiObject, catchSignals=True ):
    ...
    #
    #   hwiObject.terminate : a bool that, when set, will cause the server \
    #                           shut down. Is set to zero during startup

    # If the hwi object does not have this field, then create it
    if hasattr( hwiObject, 'terminate' ) is False:
        hwiObject.terminate = False 
    ...
    def initServerSocket( port ):
        ...
        try:
            ...
            raise serverException( "Couldn't create socket: " + str( message[0] ) + ", " + message[1] )

        try:
            listeningSocket.bind( ( "", port ) ) # bind the socket to our port
        except socket.error:
            raise serverException(  'Couldn\'t bind listening socket to port %i. This is usually caused by the port being in use. Check this, or change the port.' % (port) )
    ...
    def SIGINT_Handler( signal, frame ):
        ...
        hwiObject.terminate = True

    if catchSignals is True:
        # Register the ctrl-c interrupt handler
        signal.signal( signal.SIGINT, SIGINT_Handler )

    print "Starting server \'%s\' on port %i" % (hwiObject.hwiName, hwiObject.hwiPort)
    ...
    try:
        ...
        while( 1 ):
            try:
                ...
                if( hwiObject.terminate is True ):
                    break
                else:
                    continue
    finally:
        ...
        print "Server \'%s\' exiting" % (hwiObject.hwiName)
